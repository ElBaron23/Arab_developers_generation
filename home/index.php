...
function getLangById($id){
    global $mydb;
    $getLangName = $mydb->prepare('SELECT * FROM languages WHERE `lang_id` = :lid');
    $getLangName->bindParam('lid' , $id);
    if ($getLangName->execute()){
        if ($getLangName->rowCount() > 0){
            return $getLangName->fetchObject()->lang_name;
        }else{
            return "Not Found Lang";
        }
    }
}
...
<div class="progress_chart btn toggle active"></div>
    ...
    <?php
        ...
        ?>
<div class="already_has_lang">

<h1>عليك انهاء لغتك الحاليه (<?=getLangById($query->fetchObject()->lang_id)?>) لتتعلم لغات اخري</h1>
